//! PSX SPU Gaussian interpolation helpers.
//!
//! SEQ playback is not MIDI playback: notes resolve through VAB programs and
//! tones, trigger PSX ADPCM sample streams, step those streams with the SPU
//! pitch counter, interpolate with the hardware Gaussian table, pass through
//! ADSR, then mix through the 24-voice SPU and reverb path.
//!
//! Table and tap order reference:
//! https://problemkaputt.de/psxspx-spu-adpcm-pitch.htm

/// The PlayStation SPU Gaussian coefficient table.
///
/// The table is indexed by the 8-bit interpolation index extracted from the
/// pitch counter. Values are signed fixed-point coefficients used with `>> 15`.
...
/// Extract the ADPCM sample index from a PSX SPU pitch counter.
pub fn sample_index(counter: u32) -> usize {
    (counter >> 12) as usize
}

/// Extract the 8-bit Gaussian interpolation index from a PSX SPU pitch counter.
pub fn interpolation_index(counter: u32) -> usize {
    ((counter >> 4) & 0xFF) as usize
}
...
pub fn step_counter(counter: u32, vx_pitch: u16) -> u32 {
    counter.wrapping_add(pitch_step(vx_pitch))
}

/// Apply PSX 4-point Gaussian interpolation to the four most recent samples.
pub fn interpolate(oldest: i16, older: i16, old: i16, newest: i16, index: usize) -> i16 {
    let i = index & 0xFF;
    let out = ((GAUSS_TABLE[0x0FF - i] as i32 * oldest as i32) >> 15)
        + ((GAUSS_TABLE[0x1FF - i] as i32 * older as i32) >> 15)
        + ((GAUSS_TABLE[0x100 + i] as i32 * old as i32) >> 15)
        + ((GAUSS_TABLE[0x000 + i] as i32 * newest as i32) >> 15);
...
mod tests {
    ...
    fn zero_input_returns_zero() {
        for i in [0, 1, 128, 255] {
            assert_eq!(interpolate(0, 0, 0, 0, i), 0);
        }
    }

    #[test]
    fn constant_input_remains_approximately_constant() {
        for i in [0, 64, 128, 192, 255] {
            let out = interpolate(12_000, 12_000, 12_000, 12_000, i);
            assert!((out - 12_000).abs() <= 200, "index {i} produced {out}");
        }
    }

    #[test]
    fn table_has_psx_hardware_sum_quirk() {
        for i in 0..=255 {
            let sum = GAUSS_TABLE[0x0FF - i] as i32
                + GAUSS_TABLE[0x1FF - i] as i32
                + GAUSS_TABLE[0x100 + i] as i32
                + GAUSS_TABLE[0x000 + i] as i32;
            assert!(
                (0x7F7F..=0x7F81).contains(&sum),
                "index {i} coefficient sum {sum:#06x} is not PSX-like"
            );
        }
    }

    #[test]
    fn interpolation_boundaries_are_stable() {
        let i0 = interpolate(-10_000, -2_000, 8_000, 16_000, 0);
        let i255 = interpolate(-10_000, -2_000, 8_000, 16_000, 255);
        assert_ne!(i0, 0);
        assert_ne!(i255, 0);
        assert!(i0.abs() <= i16::MAX);
        assert!(i255.abs() <= i16::MAX);
    }

    #[test]
    fn output_is_saturated_to_i16() {
        let out = interpolate(i16::MAX, i16::MAX, i16::MAX, i16::MAX, 128);
        assert!(out <= i16::MAX);
        let out = interpolate(i16::MIN, i16::MIN, i16::MIN, i16::MIN, 128);
        assert!(out >= i16::MIN);
    }

    #[test]
    fn pitch_counter_extracts_sample_and_interpolation_bits() {
        assert_eq!(sample_index(0x0000), 0);
        assert_eq!(sample_index(0x1000), 1);
        assert_eq!(sample_index(0x23F0), 2);
        assert_eq!(interpolation_index(0x0000), 0);
        assert_eq!(interpolation_index(0x00F0), 0x0F);
        assert_eq!(interpolation_index(0x0FF0), 0xFF);
    }

    #[test]
    fn pitch_counter_unity_advances_one_sample_per_tick() {
        let mut counter = 0;
        counter = step_counter(counter, 0x1000);
        assert_eq!(sample_index(counter), 1);
        assert_eq!(interpolation_index(counter), 0);
